...
import type { Sample, AnalysisResult } from "@/types/spectrum";
...
export function generatePDFReport(reportData: ReportData): void {
  ...
  // Title
  doc.setFontSize(18);
  doc.text("Photoluminescence Spectrum Analysis Report", 105, yPosition, {
    align: "center",
  });
  yPosition += 15;
  ...
  if (reportData.sample.modification_type) {
    doc.text(
      `Modification Type: ${reportData.sample.modification_type}`,
      20,
      yPosition,
    );
    yPosition += 6;
  ...
  if (reportData.sample.processing_temperature) {
    doc.text(
      `Processing Temperature: ${reportData.sample.processing_temperature}°C`,
      20,
      yPosition,
    );
    yPosition += 6;
  ...
  if (reportData.analysis.preprocessing_params) {
    ...
    if (params.noise_reduction) {
      doc.text(
        `Noise Reduction: Savitzky-Golay (window=${params.noise_reduction.window_length}, order=${params.noise_reduction.polynomial_order})`,
        20,
        yPosition,
      );
      yPosition += 6;
    ...
    if (params.baseline_correction) {
      doc.text(
        `Baseline Correction: ${params.baseline_correction.method}`,
        20,
        yPosition,
      );
      yPosition += 6;
  ...
  // Detected Peaks
  if (
    reportData.analysis.detected_peaks &&
    reportData.analysis.detected_peaks.length > 0
  ) {
    if (yPosition > 250) {
      doc.addPage();
      yPosition = 20;
  ...
  if (reportData.analysis.fitting_results) {
    ...
    yPosition += 6;
    doc.text(
      `R² (Goodness of Fit): ${fitting.r_squared.toFixed(4)}`,
      20,
      yPosition,
    );
    yPosition += 6;
  ...
  doc.setFontSize(10);
  doc.text(
    "This report presents the photoluminescence spectrum analysis results.",
    20,
    yPosition,
  );
  yPosition += 6;
  doc.text(
    "The data has been processed and analyzed using standard scientific methods.",
    20,
    yPosition,
  );
